...
func Asp(in_str string) string {
	return "'" + in_str + "'"
}

func ReplaceStrToInt(s string) int {
	...
	ResI, err := strconv.Atoi(string(b))
	if err != nil {
		return 0
	}
...
func FormataCPF(in_str string) string {
	return in_str[0:3] + "." + in_str[3:6] + "." + in_str[6:9] + "-" + in_str[9:11]
}
